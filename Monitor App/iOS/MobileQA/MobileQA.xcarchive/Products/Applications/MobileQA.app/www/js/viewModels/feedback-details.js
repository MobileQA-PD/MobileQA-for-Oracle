 ...
 */
define(['ojs/ojcore', 'knockout', 'jquery', 'appController', 'mbe', 'mcsconfig',
  'ojs/ojknockout', 'ojs/ojinputtext', 'ojs/ojtabs', 'ojs/ojconveyorbelt'],
  function (oj, ko, $, app, mbe, mcsconfig) {
    function FeedbackDetailsViewModel () {
      ...
      self.handleActivated = function (info) {
        ...
        mbe.invokeCustomAPI(url, 'GET', null, function (statusCode, data) {
          ...
          if (data.items.length > 0) {
            ...
            console.log('feedback: ' + JSON.stringify(value))
            self.id(value.id)
      ...
      self.getPicture = function (picture) {
        self.picture('');
        if (picture) {
          var xhr = new XMLHttpRequest()
          xhr.open('GET', picture + '/data', true)
          ...
          xhr.addEventListener('load', function () {
            if (xhr.status === 200 && xhr.response.length > 0) {
              self.picture("data:image/jpg;base64," + xhr.response);
            }
          })
          xhr.send()
        }
