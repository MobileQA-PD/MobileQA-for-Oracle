 ...
 */
/*
 * Your feedback ViewModel code goes here
 */
  ...
  function (oj, ko, $, app, mbe) {
    ...
    function FeedbackViewModel () {
      ...
      self.getApplicationList = function () {
        mbe.invokeCustomAPI('QoSFeedbackAggService/applications', 'GET', null, function (statusCode, data) {
          ...
          if (self.applications().length > 0) {
            ...
            self.getFeedbackReportStateValue()['appId'] = appId
            self.getSummary()
          }
        }, self.errorCallback)
      }
      ...
      self.getSummary = function () {
        var url = encodeURI('QoSFeedbackAggService/summary?appId=' +
      ...
      self.applicationSelected = function (feedback, ui) {
      ...
      self.versionSelected = function (feedback, data) {
        if (data.option === 'value') {
          var version = data.value[0]
          self.selectedAppVersion(version)
          self.getSummary()
          self.getFeedbackReportStateValue()['appVersion'] = version
        }
      }

      self.getFeedbackReportStateValue = function () {
        if (self.app.router.getState('feedback-report').value === undefined) {
          self.app.router.getState('feedback-report').value = {}
        };
        return self.app.router.getState('feedback-report').value
      }
      ...
      self.deviceSelected = function (feedback, ui) {
        if (ui.option === 'selection' && ui.value.length > 0) {
          ...
          self.app.router.getState('feedback-list').value = params
          self.app.router.go('feedback-list')
        }
      }

      self.osSelected = function (feedback, ui) {
        if (ui.option === 'selection' && ui.value.length > 0) {
