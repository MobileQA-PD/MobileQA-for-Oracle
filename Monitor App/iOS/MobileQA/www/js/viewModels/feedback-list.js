...
define(['ojs/ojcore', 'knockout', 'jquery', 'appController', 'mbe',
  ...
  function (oj, ko, $, app, mbe) {
    function FeedbackListViewModel () {
      ...
      self.dataSource = new oj.ArrayTableDataSource(self.feedbacks, { idAttribute: 'id' })
      // Header Config
      ...
      self.viewFeedbackDetails = function (feedback, ui) {
        if (ui.option === 'selection' && ui.items.length > 0) {
          ...
          var feedbackID = selectedIdsArray[0]

          var params = {
            'id': feedbackID
          }

          self.app.router.getState('feedback-details').value = params
          self.app.router.go('feedback-details')
        }
      }
