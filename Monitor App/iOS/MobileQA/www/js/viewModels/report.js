...
define(['ojs/ojcore', 'knockout', 'jquery', 'appController', 'mbe', 'mcsconfig', 'cloudInfo','ojs/ojdatetimepicker',
  ...
  function (oj, ko, $, app, mbe, mcsconfig, cloudInfo) {
    ...
    function getDocBaseUrl() {
      ...
      let cloudInfoString = localStorage.getItem(userId + "_cloudInfo.data")
      let jData = JSON.parse(cloudInfoString)
      console.log("헬로우 :" + JSON.stringify(jData))

      return jData['infos']['serviceInstances']['cecs']['url']
    }
    ...
    function ReportViewModel () {
      ...
      // self.DOCCS_BASE_URL = 'https://content-a522193.documents.us2.oraclecloud.com' // DEPLOY_OPTION=DOCCS_BASE_URL
      // self.DOCCS_BASE_URL = mcsconfig.mobileBackends.QoS_MBE.baseUrl
      ...
      self.generatingReport = ko.observable(false)
      ...
      self.generateReport = function () {
        ...
        let cdata = cloudInfo.loadCloudInfo(cloudInfo.loadUserId())
        ...
        mbe.invokeCustomAPI('QoSReportService/reports', 'POST', JSON.stringify(reqBody), function (statusCode, data) {
          ...
          // self.reportURI(self.DOCCS_BASE_URL + '/documents/api/1.1/files/' + data.id + '/data')
          // self.reportURI(self.DOCCS_BASE_URL + "/documents/api/1.1/files/" + data.id + "/previewPath");
